...
"@
  ...
  $reportPrompt = "Create a concise report for topic '$topic'. Include problem framing, evidence-backed analysis, and practical recommendations."
  ...
  if ($reportGen.exit_code -eq 0 -and -not [string]::IsNullOrWhiteSpace($reportArtifactId)) {
    ...
    $reportFallbackPrompt = "주제 '$topic'에 대해 한국어 브리핑 리포트를 작성해줘. 형식: 1) 배경 2) 핵심 인사이트 5개 3) 리스크 3개 4) 실행 권고안 5) 참고 근거 요약."
  ...
  $slidePrompt = "Generate a presentation slide deck for '$topic'. Emphasize key insights, trade-offs, and actionable next steps."
  ...
  if ($slideGen.exit_code -eq 0 -and -not [string]::IsNullOrWhiteSpace($slideArtifactId)) {
    ...
    $slideFallbackPrompt = "주제 '$topic' 발표용으로 10장 내외 슬라이드 아웃라인을 한국어로 작성해줘. 각 슬라이드마다 제목과 핵심 불릿 3개를 포함해줘."
